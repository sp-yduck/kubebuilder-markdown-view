...
type MarkdownViewStatus struct {
	...
	Condition MarkdownViewCondition `json:"condition"`
}

//+kubebuilder:validation:Enum=NotReady;Available;Healthy
type MarkdownViewCondition string

const (
    MarkdownViewNotReady  = MarkdownViewCondition("NotReady")
    MarkdownViewAvailable = MarkdownViewCondition("Available")
    MarkdownViewHealthy   = MarkdownViewCondition("Healthy")
)

//+kubebuilder:object:root=true
//+kubebuilder:subresource:status

// +kubebuilder:printcolumn:name="Replicas",type=int32,JSONPath=".spec.replicas"
